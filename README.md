...
- Optional "Use the last active sensor on startup" toggle (during setup or in options) so the proxy resumes with the most recently selected sensor instead of the configured default.
...
- If you disable "Use the last active sensor on startup", the proxy will fall back to the configured default sensor after a restart even if you had switched to a different preset earlier.
